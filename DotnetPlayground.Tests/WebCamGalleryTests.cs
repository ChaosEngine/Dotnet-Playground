...
namespace RazorPages
{
	...
	class MockHttpClient
	{
		...
		protected virtual Task<HttpResponseMessage> GetMockResponse(HttpRequestMessage request, CancellationToken cancellationToken)
		{
			if (request.RequestUri.LocalPath == "/expectedPath")
			{
				...
				response.Content = new StringContent(GetAuthJson(), Encoding.UTF8, MediaTypeNames.Application.Json);
	...
	class ImageMockHttpClient : MockHttpClient
	{
		...
		protected override Task<HttpResponseMessage> GetMockResponse(HttpRequestMessage request, CancellationToken cancellationToken)
		{
			if (request.RequestUri.LocalPath.Contains("WebCam", StringComparison.InvariantCultureIgnoreCase))
			{
				...
				{
					Content = new ByteArrayContent(Convert.FromBase64String(Base64_LenaJpg))
				};
				return Task.FromResult(response);
			}
	...
	class TestableMjpgStreamerHttpClient : IMjpgStreamerHttpClient
	{
		public TestableMjpgStreamerHttpClient(HttpClient client, IWebHostEnvironment env, IMemoryCache cache, MjpgStreamerHttpClientHandler handler)
		{
		}

		public Task<FileContentResult> GetLiveImage(CancellationToken token)
		{
			return Task.FromResult(
				new FileContentResult(Convert.FromBase64String(ImageMockHttpClient.Base64_LenaJpg), MediaTypeNames.Image.Jpeg)
				{
